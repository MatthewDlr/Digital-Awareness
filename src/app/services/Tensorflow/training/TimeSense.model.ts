...
export class TimeSense implements TfModel {
  name: string = "TimeSense";
  epoch: number = 100;
  trainingData = [
  ...
  deNormalizeOutput(value: number): number {
    return value * (this.maxOutput - this.minOutput) + this.minMinutes;
  }

  private normalizeInput(value: number): number {
